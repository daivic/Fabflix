...
### Brief Explanation
- `ListServlet.java` is a Java servlet that talks to the database and get the stars. It returns a list of stars in the JSON format. 
The name of star is generated as a link to Single Star page.

- `movie-list.js` is the main Javascript file that initiates an HTTP GET request to the `ListServlet`. After the response is returned, `movie-list.js` populates the table using the data it gets.
...
- In both `SingleStarServlet.java` and `ListServlet.java`, a private DataSource reference dataSource is created with `@Resource` annotation. It is a reference to the DataSource `jdbc/moviedbexample` we registered in `web.xml`
